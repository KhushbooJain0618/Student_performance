...
@st.cache_resource
def load_model():
    reg = pickle.load(open("final_model.pkl", "rb"))  # Your final RandomForestRegressor
    return reg
...
def improvement_suggestions(inputs: dict):
    ...
    if inputs["social_media_hours"] > 3:
        tips.append(("Reduce social media hours", "Reducing distractions improves study efficiency."))
    if inputs["study_hours"] < 4:
        tips.append(("Increase focused study hours", "More consistent study time generally improves CGPA."))
    if abs(inputs["sleep_hours"] - 7) > 1:
        tips.append(("Stabilize sleep around 7 hours", "Better sleep reduces stress and improves focus."))
    if inputs["attendance"] < 75:
        tips.append(("Improve attendance consistency", "Higher attendance supports better academic outcomes."))
    if inputs["backlogs"] > 0:
        tips.append(("Prioritize clearing backlogs", "Backlogs negatively impact overall performance."))
...
X_input = pd.DataFrame([inputs])  # Assuming model trained with same feature order
...
st.subheader(f"Predicted CGPA for {name}: {predicted_cgpa:.2f}")

if gap > 0.2:
    st.success(f"Potential improvement: {gap:.2f} CGPA points")
elif gap < -0.2:
    st.warning(f"Potential decline: {abs(gap):.2f} CGPA points")
else:
    st.info("CGPA predicted to remain stable")

st.markdown("### Personalized recommendations")
if tips:
    for i, (tip, why) in enumerate(tips, 1):
        st.write(f"{i}. {tip} — {why}")
else:
    st.write("Inputs are typical. Focus on maintaining consistency.")

# -----------------------------
# Simple benchmarking charts
# -----------------------------
st.subheader("Feature benchmarking")
for f in ["attendance", "study_hours", "sleep_hours", "social_media_hours", "stress_index", "backlogs"]:
    fig, ax = plt.subplots(figsize=(6,1))
    ax.barh([0], [inputs[f]], color='skyblue')
    ax.set_xlim(0, max(inputs[f]+5, 10))
    ax.set_yticks([])
    ax.set_title(DISPLAY_NAMES.get(f, f))
    st.pyplot(fig)

# -----------------------------
# PDF report download
# -----------------------------
def make_pdf(name, current_cgpa, predicted_cgpa, inputs, tips):
    buf = BytesIO()
    c = canvas.Canvas(buf, pagesize=A4)
    width, height = A4

    c.setFont("Helvetica-Bold", 14)
    c.drawString(2*cm, height-2*cm, f"Student Performance Report: {name}")
    c.setFont("Helvetica", 10)
    c.drawString(2*cm, height-2.5*cm, f"Current CGPA: {current_cgpa:.2f} | Predicted CGPA: {predicted_cgpa:.2f}")
    ...
    for k, v in inputs.items():
        c.drawString(2*cm, y, f"{DISPLAY_NAMES.get(k,k)}: {v:.2f}")
        y -= 0.4*cm
    ...
    if tips:
        for i, (tip, why) in enumerate(tips,1):
            c.drawString(2*cm, y, f"{i}. {tip} — {why}")
            y -= 0.4*cm
    else:
        c.drawString(2*cm, y, "No recommendations. Inputs are typical.")

    c.save()
    buf.seek(0)
    return buf

pdf_bytes = make_pdf(name, current_cgpa, predicted_cgpa, inputs, tips)
st.download_button(
    label="Download PDF report",
    data=pdf_bytes,
